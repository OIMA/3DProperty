...
public class DAOImpl extends DaoSupport implements DAO {
     ...
     * @return List, que es el resultado de la consulta OJO!!! puede devolver un
     * vacio
     * @throws Exception
     */
    public List consultaPorCampoEspecifico(String campo, Object contenido, String predicado, Class clase, String[] orderBy) throws Exception {
        ...
        try {
            ...
            } else if (predicado.equals("like")) {
                criterio.add(Restrictions.like(campo, "%" + contenido + "%".toLowerCase()));
            }
    ...
    /**
     * 
     * @param campos arreglo[nombreUsuario,idUsuario.idPropiedad,perrito]
     * @param contenidos arreglo ej: [cadena,entero,gatito]
     * @param predicados arreglo ej: [equal,notequal,lower]
     * @param clase
     * @param orderBy
     * @return
     * @throws Exception 
        ...
        try {
            ...
            for (String restriction : predicados) {
                ...
                if (restriction.equals("equal")) {
                    ...
                    System.out.println("Despues del equals" + criterio.list().size());
                } else if (restriction.equals("lower")) {
                    criterio.add(Restrictions.lt(campos[i], contenidos[i]));
                } else if (restriction.equals("greater")) {
                    criterio.add(Restrictions.gt(campos[i], contenidos[i]));
                } else if (restriction.equals("notequal")) {
                    criterio.add(Restrictions.ne(campos[i], contenidos[i]));
                } else if (restriction.equals("like")) {
                    criterio.add(Restrictions.like(campos[i], "%" + contenidos[i] + "%".toLowerCase()));
                }
            }
